...
def test_email_message_repr(sample_email_message):
    assert repr(sample_email_message) == "sender@example.com: Test Subject (Sun, 01 Jan 2023 12:00:00 +0000)"
...
def test_email_message_mark(sample_email_message):
    ...
    sample_email_message._imap_obj.mark.assert_called_once_with([EmailFlag.FLAGGED], '1234')
...
def test_email_message_delete(sample_email_message):
    ...
    sample_email_message._imap_obj.delete_message.assert_called_once_with('1234', 'INBOX')

def test_email_message_copy(sample_email_message):
    sample_email_message.copy('Sent')
    sample_email_message._imap_obj.copy_message.assert_called_once_with('1234', 'Sent', sample_email_message)

def test_email_message_move(sample_email_message):
    sample_email_message.move('Sent')
    sample_email_message._imap_obj.move_message.assert_called_once_with('1234', 'Sent', sample_email_message)
...
def test_email_parser_multiple_emails():
    parser = EmailParser()
    contacts = parser.parse_multiple_emails('John Doe <john@example.com>, Jane Doe <jane@example.com>')
...
def test_email_recipients():
    recipients = EmailRecipients('John Doe <john@example.com>, Jane Doe <jane@example.com>')
...
def test_email_message_parse_multipart(sample_email_obj):
    ...
    sample_email_obj.add_alternative('<p>This is an HTML test email body.</p>', subtype='html')
...
def test_email_message_parse_attachment(sample_email_obj):
    ...
    sample_email_obj.add_attachment(attachment_data, maintype='application', subtype='octet-stream', filename='test.bin')
...
def todo_test_email_message_parse_error():
    ...
    with pytest.raises(EmailParsingError):
        EmailMessage( folder='INBOX', uid='1234', flags=[EmailFlag.SEEN], email_obj=invalid_email_obj, imap_obj=Mock() )
...
def test_email_message_multipart(mock_email_obj):
    ...
    mock_email_obj.walk.return_value = [
        MagicMock(get_content_type=lambda: 'text/plain', get_payload=lambda decode=False: b"Plain text content"),
        MagicMock(get_content_type=lambda: 'text/html', get_payload=lambda decode=False: b"<p>HTML content</p>"),
    ]
