    ...
    for fn in fnames:
        if fn.suffix == '.msg':
            ...
            if msg_num == 1:
                ...
                assert email_parsed.subject == "Update: Changes to better serve our users around the world"
                assert email_parsed.date == 'Sat, 2 May 2015 13:46:01 +0000'
                assert 'outside of North America' in email_parsed.text[0]['text_normalized']
                assert len(email_parsed.html) > 0

            elif msg_num == 2:
                assert str(email_parsed.sender) == 'Надежда ДранинаfS (via Twitter) <notify@twitter.com>'
                assert email_parsed.sender.name == 'Надежда ДранинаfS (via Twitter)'
                assert email_parsed.sender.email == 'notify@twitter.com'
                assert email_parsed.cc == []
                assert email_parsed.subject == "Надежда ДранинаfS (@dranina73) is now following you on Twitter!"
                assert email_parsed.date == 'Wed, 22 Apr 2015 05:00:36 +0000'
                assert 'You have a new follower on' in email_parsed.text[0]['text_normalized']
                assert len(email_parsed.html) > 0
                ...
                assert email_parsed.cc == []
                assert email_parsed.subject == "Shipment status change for package # 23696393"
                ...
                assert str(email_parsed.sender) == 'François Schiettecatte <fschiettecatte@gmail.com>'
                ...
                assert 'Django does not implement X-Accel-Redirect' in email_parsed.text[0]['text_normalized']
                assert len(email_parsed.html) == 0

            elif msg_num == 5:
                assert str(email_parsed.sender) == \
                ...
                assert email_parsed.cc == []
                assert email_parsed.subject == "Orange API : Annule et Remplace, l'Opération de maintenance Orange API aura lieu le jeudi 26 juillet au lieu de mercredi 25 juillet"
                ...
                assert str(email_parsed.sender) == \
                    'Бугрым Андрей <random555@yandex.ru>'
                ...
                assert str(email_parsed.sender) == \
                    'Илья Красильщик <publisher@meduza.io>'
